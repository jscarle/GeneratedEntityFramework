...
namespace GeneratedEntityFramework.Tests.v8.Common;

public class AsyncLock
{
    private readonly SemaphoreSlim _reentrancy = new(1, 1);

    private int _reentrances;
    ...
    private static long AsyncStackCounter;
    ...
    public Task<bool> TryLockAsync(Action callback, TimeSpan timeout)
    {
        ...
        return @lock.TryObtainLockAsync(timeout).ContinueWith(
            state =>
            {
    ...
    public Task<bool> TryLockAsync(Action callback, CancellationToken cancel)
    {
        ...
        return @lock.TryObtainLockAsync(cancel).ContinueWith(
            state =>
            {
                if (state.Exception is AggregateException ex)
                    ExceptionDispatchInfo.Capture(ex.InnerException!).Throw();
